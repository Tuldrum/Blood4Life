...
public class Blood4LifeServerSocket extends ServerSocketTemplate {
    ...
    private void proccesGetCita(Protocol protocolRequest) {
        ...
        Cita cita = getService().find(Integer.parseInt(id));
        if (cita == null) {
            String errorJson = generateNotFoundErrorJson("información de la cita no encontrada.");
            respond(errorJson);
        } else {
    ...
    private void processPostCita(Protocol protocolRequest) {
        ...
        String response = getService().save(cita);
        respond(response);
    }

    private void getUsuarioCliente(Protocol protocolRequest) {
        String id = protocolRequest.getParameters().get(0).getValue();
        UsuarioCliente cliente = getService().findCustomer(Integer.parseInt(id));
        if (cliente == null) {
            String errorJson = generateNotFoundErrorJson("Información del cliente no encontrada.");
            respond(errorJson);
        } else {
    ...
    private void processUsuarioCliente(Protocol protocolRequest) {
    ...
    private void getLugarRecogida(Protocol protocolRequest){
        String id = protocolRequest.getParameters().get(0).getValue();
    ...
    private void proccesLugarRecogida(Protocol protocolRequest){
